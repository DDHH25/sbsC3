...
// 함수를 호출하기 전에 먼저 정의가 있어야한다
// 함수는 반환값이 없거나 무족건 1개여야한다
// 매개변수는 없거나, 여러개 지정할 수 있다.
// 호출 시에는 매개변수가 선언된 위치와 개수가 일치하게 전달해야 한다.

// americano 함수 구현하기
// money가 매개변수로 주어지면 잔수를 반환하는 함수를 구현하세요
// 아메리카노 한잔은 1500원입니다. 예를 들어 money가 2000이 주어지면
// 1을 반환하도록 합니다.
// 함수 이름: americano
// 매개변수 : money, int 형
// 반환값 : int
...
// 사칙연산 계산기 함수 구현하기(+,-,*,/,%)
// 그 외는 지원하지 않는 연산자입니다를 출력하는 함수를 구현하세요
// 함수이름: clculater
// 매개변수: 정수1, 정수2 int형
// 매개변수3: 연산자, char형

int clculater(int num1, char op, int num2)
{
    int res = 0;
    switch (op)
    {
...
// 나이(int)를 매개변수로 주면 출생연도를 반환하는 함수를 구현하세요
// (현재 2026) 23 => 2004
// 함수 : birth_year
// 매개변수:age(정수)
...
{
    const int current_year = 2026;  // const 이용하여 상수로 만들기 or #define 상수 만들기 
    return  current_year - age + 1;
}

// 상품금액을 입력하면 vat를 가산하여 최종 결제 금액을 구하는 함수를 구현하세요
// vat는 상품금액의 10%입니다
// 10000원이 입력되면 11000원이 반환하여야합니다
// 함수 이름 : calc_price
// 매개변수: price, int
// 반환값 : int 
...
// 두개의 정수를 매개변수로 받아서 두 정수의 합을 구한뒤 
